...
def rnaseq_metrics_df(analysis_dir, num_seps=1, sep="."):
    ...
    star_df = pd.DataFrame(
        {name: parse_star_file(star_file)
         for name, star_file in star_names.items()}).transpose()
    ###########################################################################
    ...
    combined_df = pd.merge(cutadapt_df, star_df,
    ...
    try:
        combined_df["Percent Repetative"] = \
